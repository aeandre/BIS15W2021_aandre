...
Note: you need to write two equal signs because one equal sign defines a new vector. This does not apply to <= and >=
...
Note: VECTORS ARE ORDERED, so it is important that you put them in the order you want to make your matrix
```{r}
box_office <- c(Philosophers_Stone, Chamber_of_Secrets, Prisoner_of_Azkaban, Goblet_of_Fire, Order_of_the_Phoenix, Half_Blood_Prince, Deathly_Hallows_1, Deathly_Hallows_2)
box_office
```

Create `harry_potter_matrix` using the `matrix()` command. We need to tell R how to organize the `box_office` vector using the `nrow` (number of rows) and `byrow` (arrange data by rows, set it to T for true or F for false) commands.

There is NO 'ncol' command. R likes to organize things by rows.
```{r}
...
```{r}
movie_titles <- c("Philosophers_Stone", "Chamber_of_Secrets", "Prisoner_of_Azkaban", "Goblet_of_Fire", "Order_of_the_Phoenix", "Half_Blood_Prince", "Deathly_Hallows_1", "Deathly_Hallows_2")
movie_titles
```
...
```{r}
rownames(harry_potter_matrix) <- movie_titles
```
...
```{r}
global_earnings<- rowSums(harry_potter_matrix)
global_earnings
```

And even add a new column to reflect this calculation. `cbind()` adds columns to an existing matrix.You can also use a 'rbind()' command to add rows.
```{r}
all_harry_potter_matrix <- cbind(harry_potter_matrix, global_earnings)
...
```{r}
complete_harry_potter_matrix <-rbind(all_harry_potter_matrix, regional_earnings)
complete_harry_potter_matrix
```
