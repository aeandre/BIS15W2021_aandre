...
```{r}
homerange$locomotion <- as.factor(homerange$locomotion)
homerange$trophic.guild <- as.factor(homerange$trophic.guild)
homerange$thermoregulation <- as.factor(homerange$thermoregulation)
```

```{r}
homerange %>% 
  ggplot(aes(x = trophic.guild, fill = locomotion)) +
  geom_bar(position = position_fill()) + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = thesis_palette) +
  labs(title = "Trophic Guild by Locomotion Type",
       x = "Trophic Guild",
       y = NULL) +
  theme_linedraw() +
...
```{r}
homerange %>% 
  ggplot(aes(x = thermoregulation, fill = locomotion)) +
  geom_bar(position = position_fill()) + 
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(values = thesis_palette) +
  labs(title = "Thermoregulation by Locomotion Type",
       x = "Thermoregulation",
       y = NULL) +
  theme_linedraw() +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ggplot(homerange, aes_string(x = "locomotion", fill=input$x)) + geom_bar(position = "dodge") + theme_light(base_size = 18)
  })
...
ui <- fluidPage(    
  ...
  sidebarLayout(      
    ...
    sidebarPanel(
      selectInput("Locomotion", " Select Locomotion Type of Interest:", 
