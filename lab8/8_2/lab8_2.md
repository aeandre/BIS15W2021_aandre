...
```
## here() starts at /Users/allyandre/Desktop/BIS 15L
```
...
No. The column names are dates, which are data. 
...
```r
heartrate2 <- read_csv(here("lab8", "data", "heartrate2.csv"))
```
...
```r
tb_data <- read_csv(here("lab8", "data", "tb_data.csv"))
```
...
```r
edu_level <- read_csv(here("lab8", "data", "education_level.csv"))
```
