...
They are untidy because the income level doesn't have its own column and each observation doesn't have its own row, as they are grouped based on religion.
...
Some (but not all) of the column names are data. (The week numbers are data) We also have NA's.
```{r}
setwd("~/Desktop/BIS 15L/lab8/data")
...
They are untidy because there are data in some of the column names (days). Also, we need to remove the "day" prefix.
