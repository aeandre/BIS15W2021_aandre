...
Answer: NAs are only registering with a single variable, but there are -999s and 0s in some of the other columns. I would change the -999s and 0s to NAs given what we learned in the last lab so that we would be able to treat them all the same.
...
Answer: No. This means that they didn't live at all, which seems unlikely to be relevant to this life history data.
```{r}
...
Answer: Holothuroidea, Dipneusti, Bivalvia, Gastropoda, and Elasmobranchii have greater than 50% missing.
```{r}
cites_tidy %>% 
  group_by(class) %>% 
