...
To add a range of columns use `start_col:end_col`. Note: 'start_col' and 'end_col' are the names of the columns on either end of the chunk of interest
...
The - operator is useful in select. It allows us to select everything except the specified variables. Essentially, it is a minus.
...
When columns are sequentially named, `starts_with()` makes selecting columns easier. This allows you to select a column using only part of its name.
...
3. matches() = Select columns that match a regular expression  (see explanation below)
...
2. Are there any NAs? No! Are you sure? Yes! Try taking an average of `max. life` as a test.
...
## [1] FALSE
```
Because there is a mean, this doesn't contain any NAs.
...
## -490.2556
```
Note: this is another way of seeing the mean of mammals.
