...
## `filter()`
Unlike `select()`, `filter()` allows us to extract data that meet specific criteria within a variable. (Select allows you to extract specific data columns/variables) Let's say that we are interested only in the fish that occur in lake "AL". We can use `filter()` to extract these observations.  
...
Notice that the `|` operator generates a different result. This means 'and/or.'
...
Rules:  
+ `filter(condition1, condition2)` will return rows where both conditions are met.  (overlap of the venn diagram)
+ `filter(condition1, !condition2)` will return all rows where condition one is true but condition 2 is not.  (just one circle, minus the overlap)
+ `filter(condition1 | condition2)` will return rows where condition 1 and/or condition 2 is met. (whole venn diagram) 
+ `filter(xor(condition1, condition2)` will return all rows where only one of the conditions is met, and not when both conditions are met.  (venn diagram, minus the shared area)
