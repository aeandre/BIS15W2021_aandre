...
Now we set the bounding box. "f" expands the box a little bit beyond the bounds.
```{r}
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
...
```{r}
ggmap(map1) + 
  geom_point(data = spiders, aes(longitude, latitude), size = 2, color = "maroon", alpha = 0.7) +
  labs(x = "Longitude", y = "Latitude", title = "Spider Locations")
```
